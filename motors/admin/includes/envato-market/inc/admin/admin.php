...
if ( ! class_exists( 'Envato_Market_Admin' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_Admin {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'motors' ), '1.0.0' );
		}
		...
		public function maybe_delete_transients() {
			if ( isset( $_POST[ envato_market()->get_option_name() ] ) ) {
				...
				if ( isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( $_POST['_wpnonce'], envato_market()->get_slug() . '-options' ) ) {
		 			wp_die( __( 'You do not have sufficient permissions to delete transients.', 'motors' ) );
				}
		...
		public function admin_enqueue_script() {
			...
			wp_enqueue_script( $slug, $plugin_url . 'js/envato-market' . $min . '.js', array( 'jquery', 'jquery-ui-dialog' ), $version, true );
			wp_enqueue_script( $slug . '-updates', $plugin_url . 'js/updates' . $min . '.js', array( 'jquery', 'updates', 'wp-a11y' ), $version, true );
			...
			$exports = array(
				...
				'i18n' => array(
					'save' => __( 'Save', 'motors' ),
					'remove' => __( 'Remove', 'motors' ),
					'cancel' => __( 'Cancel', 'motors' ),
					'error' => __( 'An unknown error occurred. Try again.', 'motors' ),
				),
			);
		...
		public function render_templates() {
			...
			<script type="text/html" id="tmpl-envato-market-auth-check-button">
				<a href="<?php echo esc_url( add_query_arg( array( 'authorization' => 'check' ), envato_market()->get_page_url() ) ); ?>" class="button button-secondary auth-check-button" style="margin:0 5px"><?php esc_html_e( 'Test API Connection', 'motors' ); ?></a>
			</script>

			<script type="text/html" id="tmpl-envato-market-item">
				<li data-id="{{ data.id }}">
					<span class="item-name"><?php esc_html_e( 'ID', 'motors' ); ?>: {{ data.id }} - {{ data.name }}</span>
					<button class="item-delete dashicons dashicons-dismiss">
						<span class="screen-reader-text"><?php esc_html_e( 'Delete', 'motors' ) ?></span>
					</button>
			...
			<script type="text/html" id="tmpl-envato-market-dialog-remove">
				<div id="envato-market-dialog-remove" title="<?php esc_attr_e( 'Remove Item', 'motors' ) ?>">
					<p><?php esc_html_e( 'You are about to remove the connection between the Envato Market API and this item. You cannot undo this action.', 'motors' ) ?></p>
				</div>
			</script>

			<script type="text/html" id="tmpl-envato-market-dialog-form">
				<div id="envato-market-dialog-form" title="<?php esc_attr_e( 'Add Item', 'motors' ) ?>">
					<form>
						<fieldset>
							<label for="token"><?php esc_html_e( 'Token', 'motors' ) ?></label>
							<input type="text" name="token" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato API Personal Token.', 'motors' ); ?></p>
							<label for="id"><?php esc_html_e( 'Item ID', 'motors' ) ?></label>
							<input type="text" name="id" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato Item ID.', 'motors' ); ?></p>
						...
						<div class="envato-card-top">
							...
							<div class="column-name">
								<h4>
									...
									<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'motors' ); ?>"><?php esc_html_e( 'Version', 'motors' ); ?> {{ data.version }}</span>
								</h4>
							</div>
							<div class="column-description">
								...
								<p class="author">
									<cite><?php esc_html_e( 'By', 'motors' ); ?> {{ data.author }}</cite>
								</p>
							</div>
						</div>
						<div class="envato-card-bottom">
							<div class="column-actions">
								<a href="{{{ data.install }}}" class="button button-primary">
									<span aria-hidden="true"><?php esc_html_e( 'Install', 'motors' ); ?></span>
									<span class="screen-reader-text"><?php esc_html_e( 'Install', 'motors' ); ?> {{ data.name }}</span>
								</a>
							</div>
						</div>
					</div>
		...
		public function register_settings() {
			...
			add_settings_section(
				...
				__( 'Global OAuth Personal Token', 'motors' ),
			...
			add_settings_section(
				...
				__( 'Single Use OAuth Personal Tokens', 'motors' ),
		...
		public function install_plugin( $plugin ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'motors' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install plugins on this site.', 'motors' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'motors' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'motors' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'motors' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'motors' ) . '</a>
				</div>';
				wp_die( $msg );
			}

			$title = sprintf( __( 'Installing Plugin: %s', 'motors' ), $api->name . ' ' . $api->version );
		...
		public function install_theme( $theme ) {
			if ( ! current_user_can( 'install_themes' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'motors' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install themes on this site.', 'motors' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Theme Installer', 'motors' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'motors' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'motors' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Plugin Installer', 'motors' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			$title = sprintf( __( 'Installing Theme: %s', 'motors' ), $api->name . ' ' . $api->version );
		...
		public function ajax_upgrade_theme() {
			...
			if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
				$status['oldVersion'] = sprintf( __( 'Version %s', 'motors' ), $theme_data->get( 'Version' ) );
			}

			if ( ! current_user_can( 'update_themes' ) ) {
				$status['error'] = __( 'You do not have sufficient permissions to update themes for this site.', 'motors' );
			...
			if ( is_array( $result ) && ! empty( $result[ $theme ] ) ) {
				...
				if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
					$status['newVersion'] = sprintf( __( 'Version %s', 'motors' ), $theme_data->get( 'Version' ) );
				}
				...
				$status['error'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'motors' );
		...
		public function ajax_add_item() {
			...
			} elseif ( 'POST' !== apply_filters('stm_get_global_server_val', 'REQUEST_METHOD') ) {
				status_header( 405 );
				wp_send_json_error( 'bad_method' );
			} elseif ( empty( $_POST['token'] ) ) {
				wp_send_json_error( array( 'message' => __( 'The Token is missing.', 'motors' ) ) );
			} elseif ( empty( $_POST['id'] ) ) {
				wp_send_json_error( array( 'message' => __( 'The Item ID is missing.', 'motors' ) ) );
			}
			...
			if ( false === $request ) {
				wp_send_json_error( array( 'message' => __( 'The Token or Item ID is incorrect.', 'motors' ) ) );
			}

			if ( false === envato_market()->api()->download( $_POST['id'], $args ) ) {
				wp_send_json_error( array( 'message' => __( 'The item cannot be downloaded.', 'motors' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An unknown error occurred.', 'motors' ) ) );
		}

...
<?php endif; ?>
		...
		<form method="post" class="login">
			...
			<p class="form-row form-row-wide">
                ...
                <?php endif; ?>
					<input type="text" class="input-text" name="username" id="username" value="<?php if ( ! empty( $_POST['username'] ) ) echo esc_attr( $_POST['username'] ); ?>" placeholder="<?php esc_attr_e( 'Username or email address', 'motors' ); ?> *" />
                <?php if(stm_is_rental()) : ?>
				</div>
                <?php endif; ?>
			</p>
		...
		<form method="post" class="register">
			...
			<?php if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) : ?>
				<p class="form-row form-row-wide">
					<?php if(stm_is_rental()) : ?>
						...
						<div class="stm-rent-text-wrap">
					<?php endif ?>
					<input type="text" class="input-text" name="username" id="reg_username" value="<?php if ( ! empty( $_POST['username'] ) ) echo esc_attr( $_POST['username'] ); ?>" placeholder="<?php esc_attr_e( 'Username', 'motors' ); ?> *" />
					<?php if(stm_is_rental()): ?>
						</div>
					<?php endif; ?>
				</p>
			...
			<p class="form-row form-row-wide">
				<?php if(stm_is_rental()) : ?>
					...
					<div class="stm-rent-text-wrap">
				<?php endif ?>
				<input type="email" class="input-text" name="email" id="reg_email" value="<?php if ( ! empty( $_POST['email'] ) ) echo esc_attr( $_POST['email'] ); ?>" placeholder="<?php esc_attr_e( 'Email address', 'motors' ); ?> *" />
				<?php if(stm_is_rental()): ?>
					</div>
				<?php endif; ?>
			</p>
			...
			<!-- Spam Trap -->
			<div style="<?php echo ( ( is_rtl() ) ? 'right' : 'left' ); ?>: -999em; position: absolute;"><label for="trap"><?php esc_html_e( 'Anti-spam', 'motors' ); ?></label><input type="text" name="email_2" id="trap" tabindex="-1" /></div>
